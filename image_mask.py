...
def bbox_image(image_file):
    ...
    with open(image_file, 'rb') as f:
        ...
        if sum(sum_vector[:int(len(sum_vector) / 2)]) > sum(sum_vector[int(len(sum_vector) / 2):]):
            x -= int(0.02 * len(maskBGR[0]))
            w += int(0.02 * len(maskBGR[0]))
        else:
            w += int(0.02 * len(maskBGR[0]))
        return x, y, w, h


def crop_image(x, y, w, h, image_file):
    with open(image_file, 'rb') as f:
        original = cv2.imdecode(np.frombuffer(f.read(), dtype=np.uint8), cv2.IMREAD_COLOR)
        crop = original[y:y + h, x:x + w]
        output_file = os.path.dirname(image_file) + "\\processed\\" + image_file.split("\\")[-1]
...
def get_rotation_angle(image_file):
    ...
    src = cv2.imread(image_file + "_p.png")
    ...
    print("rectangular: ", angle_rect, "angle", angle)
    # cv2.drawContours(src, contours, contours.index(contour), (0, 0, 255), 2)
    # rot_mat = cv2.getRotationMatrix2D(cntr, -angle, 1.0)
    rot_mat = cv2.getRotationMatrix2D(cntr, -angle, 1.0)
    return cv2.warpAffine(cv2.imread(image_file), rot_mat, src.shape[1::-1], flags=cv2.INTER_LINEAR)
